...
resource "null_resource" "integration_test_setup" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "printf '[default]\nregion = us-west-2' | sudo tee ~/.aws/config",
      "printf '[default]\naws_access_key_id=%s\naws_secret_access_key=%s\naws_session_token=%s' $(aws sts assume-role --role-arn arn:aws:iam::506463145083:role/CloudWatchAgentServerRoleOnPrem --role-session-name test --query 'Credentials.[AccessKeyId,SecretAccessKey,SessionToken]' --output text) | sudo tee ~/.aws/credentials>/dev/null",
      "sudo cat ~/.aws/config",
      "sudo cat ~/.aws/credentials",
      "sudo cat /.aws/config",
      "sudo cat /.aws/credentials",
